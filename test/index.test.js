...
run.methods.forEach((method) => {
  describe(method, () => { // eslint-disable-line jest/valid-title
...

    {
      const assertError = (err) => {
        ...
        // On non-Windows OS, enoent hook is not used. Errno is -2 instead of 'ENOENT'.
        // On Windows, hook is not triggered if event is already 'error'. Errno is -4058.
        expect([ 'ENOENT', -2, -4058 ]).toContain(err.errno);
    ...
    if (run.isMethodSync(method)) {
      ...
      it('should NOT emit `error` if the command actual exists but exited with 1', async () => {
        await new Promise((resolve, reject) => {
          ...
          const onExit = jest.fn(() => { /* Do nothing */ });
          let timeout;
